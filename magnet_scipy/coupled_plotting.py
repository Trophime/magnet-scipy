...
def prepare_coupled_post(sol, coupled_system, mode: str = "regular") -> Tuple[np.ndarray, Dict]:
    """
    ...
    if n_circuits > 1:
        ...
        for i in range(n_circuits):
            for j in range(i+1, n_circuits):
                coupling_ratio = abs(M_matrix[i,j]) / np.sqrt(L_diagonal[i] * L_diagonal[j])
                coupling_ratios.append(coupling_ratio)
                print(f"  Coupling ratio {circuit_ids[i]}-{circuit_ids[j]}: {coupling_ratio:.3f}")
...
def save_coupled_results(
    ...
    if 'reference' in results[first_circuit_id] and 'error' in results[first_circuit_id]:
    ...
    current_matrix = np.array([results[cid]["current"] for cid in coupled_system.circuit_ids])
    correlation_matrix = np.corrcoef(current_matrix)
...
def create_coupling_comparison_plot(
    ...
    for i, circuit_id in enumerate(circuit_ids):
        ...
        # Current comparison
        axes[0].plot(time, results_uncoupled[circuit_id]['current'], 
                    color=color, linestyle='--', alpha=0.7,
                    label=f'{circuit_id} Uncoupled')
        axes[0].plot(time, results_coupled[circuit_id]['current'],
                    color=color, linestyle='-', linewidth=2,
                    label=f'{circuit_id} Coupled')
        
        # Voltage comparison
        axes[1].plot(time, results_uncoupled[circuit_id]['voltage'],
                    color=color, linestyle='--', alpha=0.7)
        axes[1].plot(time, results_coupled[circuit_id]['voltage'],
                    color=color, linestyle='-', linewidth=2)
        
        # Power comparison
        axes[2].plot(time, results_uncoupled[circuit_id]['power'],
                    color=color, linestyle='--', alpha=0.7)
        axes[2].plot(time, results_coupled[circuit_id]['power'],
                    color=color, linestyle='-', linewidth=2)
        
        # Error comparison (if available)
        if 'error' in results_uncoupled[circuit_id] and 'error' in results_coupled[circuit_id]:
            axes[3].plot(time, np.abs(results_uncoupled[circuit_id]['error']),
                        color=color, linestyle='--', alpha=0.7)
            axes[3].plot(time, np.abs(results_coupled[circuit_id]['error']),
                        color=color, linestyle='-', linewidth=2)
    ...
    if 'error' in results_uncoupled[circuit_ids[0]]:
        ...
        for i, circuit_id in enumerate(circuit_ids):
            color = colors[i]
            current_diff = np.abs(results_coupled[circuit_id]['current'] - 
                                results_uncoupled[circuit_id]['current'])
            axes[3].plot(time, current_diff, color=color, linewidth=2,
...
def plot_region_analysis(coupled_system, t: np.ndarray, results: Dict):
    ...
    fig, axes = plt.subplots(n_circuits + 1, 1, figsize=(16, 4 * (n_circuits + 1)), sharex=True)
    ...
    for i, circuit_id in enumerate(coupled_system.circuit_ids):
        ...
        for j, region in enumerate(sampled_regions):
            if region != prev_region:
                ...
                ax.axvspan(region_start, region_end, alpha=0.3,
                          color=region_colors.get(region.lower(), 'lightgray'))
                prev_region = region
        
        # Plot current and reference
        ax.plot(t, data['current'], color=colors[i], linewidth=2, 
               label=f'{circuit_id} Current')
        ax.plot(t, data['reference'], color=colors[i], linewidth=2, 
               linestyle='--', alpha=0.7, label=f'{circuit_id} Reference')
    ...
    for i, circuit_id in enumerate(coupled_system.circuit_ids):
        data = results[circuit_id]
        ax.plot(t, data['Kp'], color=colors[i], linewidth=2, 
               linestyle='-', label=f'{circuit_id} Kp')
        ax.plot(t, data['Ki'], color=colors[i], linewidth=2,
               linestyle=':', label=f'{circuit_id} Ki')
        ax.plot(t, data['Kd'] * 100, color=colors[i], linewidth=2,
               linestyle='-.', label=f'{circuit_id} Kd×100')
...
def analyze_magnetic_coupling_efficiency(coupled_system, results: Dict, t: np.ndarray):
    ...
    for i in range(n_circuits):
        for j in range(i + 1, n_circuits):
            ...
            cross_corr = np.correlate(current_i - np.mean(current_i), 
                                    current_j - np.mean(current_j), mode='full')
            ...
            coupling_analysis['coupling_effectiveness'][f'{circuit_i_id}_{circuit_j_id}'] = {
                'current_correlation': float(correlation),
                'coupling_strength_ratio': float(abs(M_ij) / np.sqrt(M_matrix[i,i] * M_matrix[j,j])),
                'energy_coupling_ratio': float(total_mutual_energy / 
                    (np.sum(results[circuit_i_id]['power']) * (t[1] - t[0])))
            }
            
            coupling_analysis['phase_relationships'][f'{circuit_i_id}_{circuit_j_id}'] = {
